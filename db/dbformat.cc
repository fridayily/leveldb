...
int InternalKeyComparator::Compare(const Slice& akey, const Slice& bkey) const {
  ...
  //    sequence num 降序，sequence number 越大值越新
  //    decreasing type (though sequence# should be enough to disambiguate)
  //    type 降序，值大表示删除
  int r = user_comparator_->Compare(ExtractUserKey(akey), ExtractUserKey(bkey));
  if (r == 0) {
    // 先比较 sequence number + type 形成的数字，虽然只比较 sequence 就足够了
    // 取出 sequence number 和 type
    const uint64_t anum = DecodeFixed64(akey.data() + akey.size() - 8);
    const uint64_t bnum = DecodeFixed64(bkey.data() + bkey.size() - 8);
    if (anum > bnum) {
      // anum>bnum 表示是 a 比 b 更新
      r = -1;
    } else if (anum < bnum) {
      r = +1;
    }
...
void InternalKeyComparator::FindShortestSeparator(std::string* start,
  ...
  // 如果相比较的字符串相等，tmp 和 user_start 大小相等
...
void InternalFilterPolicy::CreateFilter(const Slice* keys, int n,
  ...
  for (int i = 0; i < n; i++) {
    // 将internal_key 转为 user_key, 还是原地址
...
LookupKey::LookupKey(const Slice& user_key, SequenceNumber s) {
  ...
  char* dst;
  // 如果预估的key长度小于200，则将dst指向结构体中已分配的空间
